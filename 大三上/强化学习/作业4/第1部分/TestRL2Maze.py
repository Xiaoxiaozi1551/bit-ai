...
# Test REINFORCE
reinforce = RL2.RL2(mdp, np.random.normal)

rewardList_sum = np.zeros(200)
for i in range(10):
    [policy, rewardList] = reinforce.reinforce(s0=0, initialPolicyParams=np.random.rand(mdp.nActions, mdp.nStates),
                                                    nEpisodes=200, nSteps=100)
    rewardList_sum += rewardList
print("Reinforce:rewardList_avg", rewardList_sum / 10)
...
fig = plt.figure()
X = np.arange(1, 201)
plt.plot(X, rewardList_sum / 10, label="reinforce")
...
plt.title("周圣威's result")
plt.legend()
plt.savefig('reinforce and q-learning.jpg')
plt.show()
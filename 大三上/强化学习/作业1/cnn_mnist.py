...
class Cnn_net(nn.Module):
    def __init__(self):
        ...
        self.layer1 = nn.Sequential(
            nn.Conv2d(in_channels=1, out_channels=32, kernel_size=5, stride=(1,1), padding=2),
            nn.ReLU(),
        ...
        self.layer2 = nn.Sequential(
            nn.Conv2d(in_channels=32, out_channels=64, kernel_size=5, stride=(1,1), padding=2),
            nn.ReLU(),
            nn.MaxPool2d(kernel_size=2, stride=2)
        )
        # 设置正则化层
...
def train(x_train, y_train, x_test, y_test, BATCH_SIZE, model, loss_function, optimizer):
    ...
    print("周圣威's procedure has been completed")#将XXX改为你的中文名字，最终提交运行截图

...
class NatureQN(DQN):
    ...
    def initialize_models(self):
        ...
        self.q_network = nn.Sequential(
            ...
            nn.ReLU(),
            nn.Flatten(),
            nn.Linear(64 * img_width * img_width, 512),
            nn.ReLU(),
    ...
    def calc_loss(self, q_values : torch.tensor, target_q_values : torch.tensor,
        ...
        # 计算 target Q-values
        target_q_values = rewards + gamma * torch.max(target_q_values, dim=1).values * (~done_mask)

        # 选取动作对应的 Q-values
        q_values = torch.sum(q_values * F.one_hot(actions.to(torch.int64), num_actions), dim=1)

...
from Exp_Model import BiLSTM_model, Transformer_model, Transformer_model_pre
...
def train():
    ...
    for epoch in range(num_epochs):
        ...
        for data in tqdm_iterator:
            ...
            print(batch_x.size())
            # batch_x1, batch_x2, batch_x3, batch_y = data[0].to(device), data[1].to(device), data[2].to(device), data[3].to(device)

            # 模型预测
            y_hat = model(batch_x)
            # y_hat = model(batch_x1, batch_x2, batch_x3)
            # print(y_hat)
            # print(batch_y)
...
if __name__ == '__main__':
    ...
    #-----------------------------------------------------begin-----------------------------------------------------#
    # 可修改选择的模型以及传入的参数
    # model = BiLSTM_model(vocab_size=vocab_size, ntoken=max_token_per_sent, d_emb=embedding_dim, embedding_weight=dataset.embedding_weight).to(device) # 双向lstm
    model = Transformer_model(vocab_size=vocab_size, ntoken=max_token_per_sent, d_emb=embedding_dim, nhead=6, embedding_weight=dataset.embedding_weight).to(device) # Transformer
    # model = Transformer_model_pre(vocab_size=vocab_size, ntoken=max_token_per_sent, d_emb=768, nhead=6).to(device)
